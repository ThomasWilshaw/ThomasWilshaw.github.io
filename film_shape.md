...
Ektachrome was first released by Kodak in the 1940s and survived in various forms
...
desired and seemed to be missing a lot of the colourfulness and punchyness 
inherent in Ektachrome. As it turned out however, a friend happened to be 
experimenting modelling with modelling film print papers and with a lot of help 
...
sense of the general shape and relationship between the three dyes. For example
...
an increase or decrease in exposure causes a similar 
...
These curves are much more intuitive (to me at least) and tell us more about how
...
with Dmax at the extremities and Dmin in the centre. This highlights several
very important factors that make film so special.
...
and shape of the film's gamut changes dramatically, and it gracefully attenuates
to the achromatic point. You can also see this in the transmittance graphs, as
the densities decrease all the dyes tend toward a flat pass filter. One way to
visualize this is to imagine drawing a line connecting corresponding density
points on each curve. The set of shrinking polygons this would make is an
approximation of the gamut at each density step. As the size of the available
gamut shrinks the reproducible chrominance gets smaller and smaller meaning even
a very bright red laser would ultimately only produce a fairly desaturated red.

Secondly the plot shows the hue flights of the film, as the density of each dye
changes its hue does not follow a straight line through the CIE xyY space but 
...
All film stocks exhibit these two features and this is part of what gives each 
...
And here is the film hull compared to the Rec.709 gamut. You can again see the slightly suspect looking yellows which do not conform to the overall closed volume one would intuitively expect. 
...
appear to follow roughly the same hue flights and at DMax reach similarly far 
the CIE xy space. 
...
![The normalised spectral response curves of the scanner's camera](/docs/assets/images/scanner_normalised_response.jpg)
![The red, green and blue LED spectral power distributions](/docs/assets/images/scanner_led_spd.jpg)
...
It is immediately obvious that the two are very different looking, for example:
...
deep and pure
- The yellow and purple tones are practically none existent
- The rate of change of attenuation is much slower in the scanner observer sweep
- The scanner observer sweep is missing the hue flights of the mixtures, e.g.
the green/yellow mixtures push toward yellow as the density reduces, as do the
red/yellows in the Standard Observer sweep.

All this leads to an image that feels much less contrasty and is missing all lot
of the subtleties and textures apparent when looking at the film on a light box.
Whilst I can't currently show any example frames here it is very interesting to 
...
results, a lot of the missing punchyness and colour contrast that was the 
original reason for choosing the film had returned. The main issue currently are
the yellow hues, they are coming out extremely saturated and bright. Whilst this
...
order of operations. I'm hopeful that generating the sweeps in transmittance 
